...
if __name__ == "__main__":
    ...
    if valid_or_test==1:
        ...
        fp_all = []
        fn_all = []
        for scores,x  in  enumerate(all_scores):
            ...
            iou_avg = np.average(p[:,0][p[:,0]!=1 or p[:,0]!=0])
            c2_avg = np.average(p[:,1][p[:,1]!=0])
            c3_avg = np.average(p[:,2][p[:,2]!=0])
...
#            c3 = np.array(c3)
#            c3[~np.isfinite(c3)] = 0
            slice_no = list(map(str,np.arrange(0,len(p))))
